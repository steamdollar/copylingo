...
`cmd/ja/kana_seeder`가 문항을 하나씩 insert하던 구조를 `transaction + batch insert`로 변경해 성능과 원자성을 함께 개선했습니다.
...
- `cmd/ja/kana_seeder/main.go`는 per-question insert 대신 먼저 `[]*model.Question`을 구성한 뒤 한 번에 저장하도록 변경했습니다.
