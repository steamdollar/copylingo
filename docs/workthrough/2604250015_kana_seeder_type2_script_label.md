...
- `.gitignore`
  - root binary ignore 의도를 유지하면서 `cmd/kana_seeder/main_test.go`가 숨겨지지 않도록 `server`, `kana_seeder` 패턴을 root anchored 패턴으로 변경했다.
- `cmd/kana_seeder/main_test.go`
