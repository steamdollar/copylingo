...
### 1.1. 프로젝트 성격 및 설계 기준 (중요)
- Copylingo는 **(a) 실제 외국어 학습용 + (b) 포트폴리오**의 dual-purpose 프로젝트입니다. 우선순위는 **(b) 포트폴리오**.
...
- 기존 "개인 사용 최적화" 원칙(소셜/과금 기능 불필요 등)은 **기능 범위(scope)** 결정에만 적용되고, **아키텍처/성능 결정**에는 적용하지 않습니다.
