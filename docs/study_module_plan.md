...
> 작성일: 2026-06-02  
> 상태: Task 1 구현 완료. Task 2~3을 순차 구현하고, Task 4는 별도 설계 합의 전까지 보류한다.
...
- 권장안: `sessions.mode VARCHAR(20) NOT NULL DEFAULT 'quiz'`를 추가한다.
...
- 기존 Quiz Session은 Default `mode='quiz'`로 하위 호환성을 유지한다.
...
- `다음` 동작 시 해당 `session_materials.studied_at`을 기록한다.
- 마지막 카드 처리 시 `sessions.status='completed'`, `completed_at=NOW()`를 반영한다.
- Redis Quiz Working Set은 재사용하지 않는다. Study 상태 관리 방식은 UX 확정 후 판단한다.
- Scheduler 자동 생성과 “오늘 학습량” 계산은 이 Task에서 구현하지 않는다.
